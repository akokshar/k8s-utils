...
func NewKubeGC(config *rest.Config, namespace string, labelSelector string, annotationFilter string) (KubeGC, error) {
	/*
		ensure lable/annotation key exists, so that selector like
		'test=test' will return only resources with label/annotation 'test' set
	*/
	filter, err := labels.Parse(annotationFilter)
	if err != nil {
		return nil, err
	}

	requirements, _ := filter.Requirements()
	for _, req := range requirements {
		addReq, err := labels.NewRequirement(req.Key(), selection.Exists, []string{})
		if err != nil {
			return nil, err
		}
		filter = filter.Add(*addReq)
	}
...
func (gc *kubeGC) String() string {
	...
	return fmt.Sprintf(
		"Scope: '%s', LabelSelector: '%s', AnnotationFilter: '%s'",
		scope,
		gc.labelSelector,
		gc.annotationFilter.String(),
	)
}
	...
	for _, group := range serverGroups.Groups {
		...
		for _, apiResource := range srs.APIResources {
			...
			for _, r := range list.Items {
				...
				if gc.annotationFilter.Empty() || !gc.annotationFilter.Matches(annotations) {
